...
mwalk <- readOGR(dsn = "data_files/mwalk.shp")
statefips <- read.csv("data_files/State FIPS.csv")
combined <- read.csv("data_files/combined.csv")
walkmi <- read.csv("data_files/walkmi.csv")
...
comparison_plot <- ggplot(data = combined, aes(x=`Location`, y=`Walkability`, fill=`Location`)) + 
